...
class _ModulesScreenState extends State<ModulesScreen> {
  ...
  Future<void> _loadQuizProgress() async {
    try {
      ...
      latestScore.forEach((quizId, scoreInt) {
        int total = 0;
        if (quizId == 'sysint_beginner_ch1') { total = BeginnerSysIntQuiz.questions.length; }
        else if (quizId == 'sysint_intermediate_ch1') { total = IntermediateSysIntQuiz.questions.length; }
        else if (quizId == 'sysint_advanced_ch1') { total = AdvancedSysIntQuiz.questions.length; }
      ...
      setState(() {
        ...
        latestScore.forEach((quizId, scoreInt) {
          int total = 0;
          if (quizId == 'sysint_beginner_ch1') { total = BeginnerSysIntQuiz.questions.length; }
          else if (quizId == 'sysint_intermediate_ch11') { total = IntermediateSysIntQuiz.questions.length; }
          else if (quizId == 'sysint_advanced_ch1') { total = AdvancedSysIntQuiz.questions.length; }
  ...
  Widget _buildModuleCard(Module module) {
    ...
    int total = 0;
    if (module.moduleId == 'sysint_beginner_ch1') {
      total = BeginnerSysIntQuiz.questions.length;
    } else if (module.moduleId == 'sysint_intermediate_ch1') {
      total = IntermediateSysIntQuiz.questions.length;
    } else if (module.moduleId == 'sysint_advanced_ch1') {
      total = AdvancedSysIntQuiz.questions.length;
    }
